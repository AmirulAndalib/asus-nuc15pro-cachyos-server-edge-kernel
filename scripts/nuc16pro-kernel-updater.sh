...
# written here. This box has custom BIOS power limits and fan curves; writing
# RAPL or platform_profile from the OS would override them, so BIOS/firmware
# keeps full ownership of power and fan/thermal policy. The 356H is hard-capped
# at its 80W Maximum Turbo Power (Intel spec) regardless, confirmed on-device
# (package stays <=80W under stress even with RAPL raised to 104W), so an OS RAPL
# write would only stomp the BIOS values for zero gain. The updater still reads
# the RAPL constraints for status, so the BIOS-set PL1/PL2 remain visible.
# Everything below tunes devices WITHIN the BIOS envelope and never changes
# power limits or fan curves.
